#!/usr/bin/env python3
"""
Threshold Optimization for RetinaSense ViT
==========================================
Apply threshold optimization to ViT-Base-Patch16-224 model
"""
...
MODEL_PATH = './outputs_vit/best_model.pth'
OUTPUT_DIR = Path('./outputs_vit')
CACHE_DIR = './preprocessed_cache_vit'  # ViT uses separate cache
IMG_SIZE = 224  # ViT uses 224×224
BATCH_SIZE = 64
NUM_WORKERS = 8
CLASS_NAMES = ['Normal', 'Diabetes/DR', 'Glaucoma', 'Cataract', 'AMD']

print("🎯 Threshold Optimization for RetinaSense ViT")
print("="*50)
...
# Add cache paths (224×224 for ViT)
cache_paths = []
...
print("\n[2/3] Loading model...")
model = MultiTaskViT(n_disease=5, n_severity=5, drop=0.4)
ckpt = torch.load(MODEL_PATH, map_location=device, weights_only=False)
model.load_state_dict(ckpt['model_state_dict'])
model = model.to(device)
model.eval()
print(f"✅ Loaded checkpoint from epoch {ckpt['epoch']}")
print(f"   Checkpoint macro F1: {ckpt.get('val_macro_f1', 0):.3f}")
...
ax.set_title('Per-Class F1: Baseline vs Optimized (ViT)')
ax.set_xticks(x)
...
ax.set_title('Optimized Thresholds per Class (ViT)')
ax.set_xticklabels(CLASS_NAMES, rotation=45, ha='right')
ax.legend()
ax.set_ylim([0, 1])
ax.grid(axis='y', alpha=0.3)
...
ax.set_title('Per-Class F1 Improvement (ViT)')
...
plt.tight_layout()
plot_path = OUTPUT_DIR / 'threshold_comparison_vit.png'
plt.savefig(plot_path, dpi=150, bbox_inches='tight')
print(f"📊 Comparison plot saved to {plot_path}")

print("\n✅ ViT threshold optimization complete!")
